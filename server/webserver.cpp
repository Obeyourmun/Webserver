...
void WebServer::_Start() {
    ...
    while(!isClose_) {
        ...
        for(int i = 0; i < eventCnt; i++) {
            ...
            if(fd == listenFd_) {   // 监听套接字 处理新连接
                DealListen_();
            }            
            else if(events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR)) {  // 事件挂起，错误或错误，关闭事件连接
